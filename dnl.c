...
static void* dnload_find_symbol(const struct link_map* lmap, int search) __attribute__(( regparm(2) ));
...
static void* dnload_find_symbol(const struct link_map* lmap_in, int search)
{
	dprintf("Link start\n");
	struct link_map* lmap = lmap_in;
